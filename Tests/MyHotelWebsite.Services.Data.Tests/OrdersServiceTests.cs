    ...
    public class OrdersServiceTests
    {
        ...
        public async Task AddOrderAsyncShouldWorkCorrectly()
        {
           ...
           new Order()
           {
            ...
            List<SingleShoppingCartViewModel> shoppingCartsEmptyList = new List<SingleShoppingCartViewModel>();

            AllShoppingCartsOfOneUserViewModel modelWithEmptyList = new AllShoppingCartsOfOneUserViewModel()
            {
                ShoppingCartsList = shoppingCartsEmptyList,
            };
            ...
            await Assert.ThrowsAsync<Exception>(() => ordersService.AddOrderAsync(modelWithEmptyList, "94d46c80-202e-46f0-b762-0f2087b29b41"));
            dbContext.Dispose();
        }
        ...
        [Fact]
        public async Task GetOrderDetailsToAddCommentAsyncShouldWorkCorrectly()
        {
            DbContextOptionsBuilder<ApplicationDbContext> optionBuilder = new DbContextOptionsBuilder<ApplicationDbContext>()
                .UseInMemoryDatabase("TestOrdersDb");
            ApplicationDbContext dbContext = new ApplicationDbContext(optionBuilder.Options);
            ...
            AutoMapperConfig.RegisterMappings(Assembly.Load("MyHotelWebsite.Web.ViewModels"));
            OrdersService ordersService = new OrdersService(ordersRepoDB, dishOrdersRepoDB, shoppingCartsRepoDB);
            HotelAdministrationAddCommentToOrderViewModel model = await ordersService.GetOrderDetailsToAddCommentAsync(2);
            Order order = ordersRepoDB.AllAsNoTracking().FirstOrDefault(x => x.Id == 2);
            Assert.Equal(string.Empty, order.Comment);
            Assert.Equal(OrderStatus.Ready, order.OrderStatus);
            Assert.Equal("2b7462c2-fcda-42b9-87f6-d1fba671afa4", order.ApplicationUserId);
            dbContext.Dispose();
        }

        [Fact]
        public void GetOrderTotalShouldWorkCorrectly()
        {
            DbContextOptionsBuilder<ApplicationDbContext> optionBuilder = new DbContextOptionsBuilder<ApplicationDbContext>()
                .UseInMemoryDatabase("TestOrdersDb");
            ApplicationDbContext dbContext = new ApplicationDbContext(optionBuilder.Options);
            ...
            EfDeletableEntityRepository<DishOrder> dishOrdersRepoDB = new EfDeletableEntityRepository<DishOrder>(dbContext);
            EfDeletableEntityRepository<ShoppingCart> shoppingCartsRepoDB = new EfDeletableEntityRepository<ShoppingCart>(dbContext);
            IEnumerable<SingleDishOrderViewModel> dishesList = new List<SingleDishOrderViewModel>()
            {
                new SingleDishOrderViewModel()
                {
                    DishId = "45a2a9c8-c7fd-4a33-9c25-fb9e9cd8acfd",
                    OrderId = 1,
                    ApplicationUserId = "2b7462c2-fcda-42b9-87f6-d1fba671afa4",
            ...
            OrdersService ordersService = new OrdersService(ordersRepoDB, dishOrdersRepoDB, shoppingCartsRepoDB);
            decimal totalSum = ordersService.GetOrderTotalAsync(dishesList);
